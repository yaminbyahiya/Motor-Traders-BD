...
if(isset($_POST['submitRegister']) && isset($_POST['email']))
{
	...
	if($p1==$p2)
	{
		...
		if($userFound==0)
		{
			...
			die("<script>alert('Thanks for registration at MotorTrader. Please login now.'); window.location='login-bn.php';</script>");
		...
		{
			mysqli_close($dbcon);
			die("<script>alert('Userid/Email already registered with MotorTrader. Please try again.'); window.location='account-bn.php';</script>");
		}
	}
	else
	{
		die("<script>alert('Password not matched. Please try again.'); window.location='account-bn.php';</script>");
	}
}
else
{
	die("<script>window.location='account-bn.php';</script>");
}
?>
