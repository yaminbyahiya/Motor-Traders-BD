	...
	if($userFound==1)
	{
		...
		if($userFound==1)
		{
			if($status=='y')
			{
				...
				die("<script>window.location='profile-bn.php';</script>");
			}
			elseif($status=='b')
			{
				echo "<script>alert('WARNING!!! You were permanently banned from MotorTrader website. Please contact with MotorTrader for details.'); window.location='login-bn.php';</script>";
			}
			elseif($status=='t')
			{
				echo "<script>alert('WARNING!!! You were temporarily banned from MotorTrader website.'); window.location='login-bn.php';</script>";
			}
			elseif($status=='n')
			{
				echo "<script>alert('ALERT!!! You were not verified yet by MotorTrader Admin.'); window.location='login-bn.php';</script>";
			}
		}
		else {
			die("<script>alert('Login password is invalid. Please try with correct one.'); window.location='login-bn.php';</script>");
		}
	}
	else
	{
		...
		mysqli_close($dbcon);
		die("<script>alert('Userid/Email did not register with MotorTrader. Please try again.'); window.location='login-bn.php';</script>");
	}
}
?>
...
<div class="page-wrapper">
    ...
    <?php $menu='login'; require('files/header-bn.php'); ?>
