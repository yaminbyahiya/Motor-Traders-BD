...
if(isset($_REQUEST['invid']) && isset($_REQUEST['tid']) && isset($_REQUEST['uid']))
{
	...
	while($row = mysqli_fetch_array($r, MYSQLI_BOTH))
	{
		...
		$sql2 = "update `$db`.`mt_tran_ssl_log` set result='CANCEL', result_dt='$now' where invid='$invid' and userid='$u' and tran_id='$tran_id';";
		mysqli_query($dbcon, $sql2) or die("$sql2");
		
		$sql2 = "update `$db`.`mt_transaction_log` set result='CANCEL', result_dt='$now' where invid='$invid' and userid='$u';";
		mysqli_query($dbcon, $sql2) or die("$sql2");
	...
	{
		die("<script>alert('WARNING !!! Payment cancelled.'); window.location='payment_after.php?uid=".str_rot13($u)."&txntype=$txntype';</script>");
	}
}
